...
function get_Init_LabelSet(Start_node::Int, Start_time::Int)
    n_l = -100
    T_l = Start_time
    S_l= 0
    Q_l = 0
    ...
    return LabelSet(n_l, T_l, S_l, Q_l, prel_l, Dwell_l, Transfer_l, BUS_station_l, preset1_ll, preset2_ll, preset1transfer_ll, BUS_station_continue_dropoff, BUS_station_start_pickup, BUS_station_forbidden)

end

#---------------------------------------------------------------------------------------#

function check_New_LabelJ(se::LabelSet, bus_jj::Int, j::Int)
    if j > n
        order = oset[j - n]
        # ========================= Feasibility check 1
...
"""
function get_New_LabelJ(se::LabelSet, e::Int, bus_jj::Int, j::Int)
...
"""
function Procedure_DP_for_add_arc(lset::Vector{LabelSet}, Start_time::Int, index_start::Int, ll::Int, allstations::Vector{Int}, alltimes::Vector{Float64}, allorders::Vector{Tuple{Int64,Int64,Base.Int64}})
    se = lset[ll]
    ...
    push!(rset, RouteSet(start_r, end_r, g_r, preseta_r, presetb1_r, presetb2_r, nodeset_r, customer_pudo_r))
end
...
"""
function Procedure_DP_set_station(lset::Vector{LabelSet}, ll::Int)
    se = lset[ll]
...
"""
function Procedure_DP_for_bus_jj(lset::Vector{LabelSet}, Start_node::Int, Start_time::Int, index_start::Int, e::Int, ll::Int, bus_jj::Int, printstatements::Int)
    se = lset[e]
    bus_ii = se.BUS_station_l
                        ...
                        if (se.n_l < j && bus_jj == bus_ii) || bus_jj != bus_ii #check that we dropoff passengers "in order", to avoid enumerating permutations of dropping off multiple passengers at the same stop
                            if bus_jj in Possiblematch_station_d[j - n] || (bus_jj in Hub_Possiblematch_station[j - n] && !(j - n in se.preset1transfer_ll)) # Conduct transfer dropoff: no transfer pickup previous
                                ...
                                if check_New_LabelJ(sl, bus_jj, j)
                                    lset[ll] = sl
                                    Procedure_DP_for_generating_arcs(lset, Start_node, Start_time, index_start, ll, printstatements)
                                end
                                ll -= 1
                            end
                        end
...
function Procedure_DP_for_generating_arcs(lset::Vector{LabelSet}, Start_node::Int, Start_time::Int, index_start::Int, ll::Int, printstatements::Int)
    ...
    if se.Q_l == 0 && se.S_l > 0
        ...
        Procedure_DP_for_add_arc(lset, Start_time, index_start, ll, allstations, alltimes, allorders)
        return
    end                                                                                             
    ...
    for bus_jj in union(bus_ii:TOTAL_Station, bus_ii - 1:-1:1)
        Procedure_DP_for_bus_jj(lset, Start_node, Start_time, index_start, e, ll, bus_jj, printstatements)
    end
...
function get_indexes_Classic()
    ...
    @inbounds for rr in (timespace_route1 + 1):r-1
        push!(R_minus[rset[rr].start_r], rr)
        push!(R_plus[rset[rr].end_r], rr)
...
function get_optimal_plan(model)
    ...
    opt_indexes = findall(x -> x > 0.99, z_r_opt)
    for rr in opt_indexes
        for j in 1:n
            #if j in rset[rr].preseta_r
            #    println("rr = ", rr, " includes customer ", j)
            #end
            for ff in 1:f
                if n_location_f[ff] == j
                    if ff in rset[rr].presetb1_r
                        println("rr = ", rr, " includes transfer pick customer ", j)
                    end
                    if ff in rset[rr].presetb2_r
...
"""
function solve_network_model(W::Int)
    ...
    #@constraint(model, vehiclestartlocats[ss in vehiclestartlocs], sum(sum(z_r[rr] for rr in R_plus_dummy[nodelookup[ss,t]]) for t in 0:0) == 1)
    ...
    return optimal_objective, gap, upper_bound, r_opt, solve_time(model), value(Λ), customers_served, customers_transferred
end
...
function solve_network_model_cg(W::Int)
    ...
    @constraint(model, flowbalance2[ss=N_dummyorig], sum(z_r[rr] for rr in R_minus_dummy[ss]) - sum(z_r[rr] for rr in R_plus_dummy[ss]) == Λ)
    @constraint(model, flowbalance3[ss=N_dummydest], sum(z_r[rr] for rr in R_minus_dummy[ss]) - sum(z_r[rr] for rr in R_plus_dummy[ss]) == -Λ)
    #@constraint(model, vehiclestartlocs[ss in vehiclestartlocs], sum(z_r[rr] for rr in R_plus[nodelookup[ss,0]]) == 1)
    @constraint(model, ordersserved[j=1:n], sum(z_r[rr] for rr in R_allpickups[j]) <= 1)
    @constraint(model, transferconsistency[j=1:n], sum(z_r[rr] for rr in R_transpickups[j]) == sum(z_r[rr] for rr in R_transdropoffs[j]))
    @constraint(model, dropoffafterpickup[ff=1:f], sum(z_r[rr] for rr in R_one[ff]) <= sum(z_r[rr] for rr in R_two[ff]))

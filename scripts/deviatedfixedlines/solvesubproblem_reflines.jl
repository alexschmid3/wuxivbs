...
function Procedure_DP_for_bus_jj_reflines(ln::Int, lset::Vector{LabelSet3}, Start_node::Int, Start_time::Int, index_start::Int, e::Int, ll::Int, bus_jj::Int, printstatements::Int, localrset::Vector{RouteSet})
    se = lset[e]
                        ...
                        if (se.n_l < j && bus_jj == bus_ii) || bus_jj != bus_ii #check that we dropoff passengers "in order", to avoid enumerating permutations of dropping off multiple passengers at the same stop
                            if bus_jj in Possiblematch_station_d[j - n] || (bus_jj in Hub_Possiblematch_station[j - n] && !(j - n in se.preset1transfer_ll)) # Conduct transfer dropoff: no transfer pickup previous
                                ...
                                if check_New_LabelJ(sl, bus_jj, j, ln)
                                    lset[ll] = sl
                                    localrset = Procedure_DP_for_generating_arcs_reflines(ln, lset, Start_node, Start_time, index_start, ll, printstatements, localrset)
                                end
                                ll -= 1
                            end
                        end
...
"""
function Procedure_DP_for_check_arc_reflines(ln::Int, lset::Vector{LabelSet3}, Start_time::Int, index_start::Int, ll::Int, allstations::Vector{Int}, alltimes::Vector{Float64}, allorders::Vector{Tuple{Int64,Int64,Base.Int64}}, localrset::Vector{RouteSet})
    se = lset[ll]
...
function Procedure_DP_for_generating_arcs_reflines(ln::Int, lset::Vector{LabelSet3}, Start_node::Int, Start_time::Int, index_start::Int, ll::Int, printstatements::Int, localrset::Vector{RouteSet})
    ...
    if se.Q_l == 0 && se.S_l > 0
        ...
        localrset = Procedure_DP_for_check_arc_reflines(ln, lset, Start_time, index_start, ll, allstations, alltimes, allorders, localrset)
        return localrset
    end                                                                                             
    ...
    for bus_jj in union(bus_ii, potentialnextstop[ln, lastcheckpoint, bus_ii]) #union(bus_ii:TOTAL_Station, bus_ii - 1:-1:1)
        localrset = Procedure_DP_for_bus_jj_reflines(ln, lset, Start_node, Start_time, index_start, e, ll, bus_jj, printstatements, localrset)
    end
...
function enumerateroutes_reflines()
    ...
    sp_rset = Dict{Int64,Vector{RouteSet}}()
    for ln in 1:numlines
        sp_rset[ln] =  Vector{RouteSet}()
    end
    ...
    for ln in 1:numlines
        for s_1 in 2:s
            if n_location_s[s_1] != TOTAL_Station + 1 && n_location_s[s_1] != 0 
                ...
                localrset = Procedure_DP_for_generating_arcs_reflines(ln, lset, n_location_s[s_1], time_s[s_1] * unit, s_1, 1, 0, localrset)
                ttm = time()-stm
                #if ttm > 0.1
                #    println("s_1 = $s_1 --> ", ttm)
                #end
...
function solvesubproblem_reflines(dualinfo, localrset)

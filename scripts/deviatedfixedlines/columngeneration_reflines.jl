...
include("solvesubproblem_reflines.jl")
include("initializenetwork_reflines.jl")
...
function initializermp()
    ...
    N_flow, N_dummyorig, N_dummydest = Dict(), Dict(), Dict()
    ...
    for ln in 1:numlines
        r = length(rset[ln])
        N_flow[ln] = [ss for ss in 1:s if n_location_s[ss] != TOTAL_Station + 1 && n_location_s[ss] != 0]
        R_minus[ln] = [Vector{Int}() for _ = 1:s]
        R_plus[ln] = [Vector{Int}() for _ = 1:s]
        N_dummyorig[ln] = [ss for ss in 1:s if n_location_s[ss] == 0]
        N_dummydest[ln] = [ss for ss in 1:s if n_location_s[ss] == TOTAL_Station + 1]
...
function columngeneration_reflines()
    ...
    z_r = Dict() 
	for ln in 1:numlines, rr in 1:length(rset[ln])
	    global z_r[ln,rr] = @variable(rmp, lower_bound = 0) #upper bound implied by ordersserved constraint                      
	    set_name(z_r[ln,rr], string("z_r[",ln,",",rr,"]")) 
	end
    ...
    @constraint(rmp, flowbalance1[ln in 1:numlines, ss=N_flow[ln]], sum(z_r[ln,rr] for rr in R_minus[ln][ss]) == sum(z_r[ln,rr] for rr in R_plus[ln][ss]));
    @constraint(rmp, flowbalance2[ln in 1:numlines, ss=N_dummyorig[ln]], sum(z_r[ln,rr] for rr in R_minus_dummy[ln][ss]) - sum(z_r[ln,rr] for rr in R_plus_dummy[ln][ss]) == 1);
    @constraint(rmp, flowbalance3[ln in 1:numlines, ss=N_dummydest[ln]], sum(z_r[ln,rr] for rr in R_minus_dummy[ln][ss]) - sum(z_r[ln,rr] for rr in R_plus_dummy[ln][ss]) == -1);
    @constraint(rmp, ordersserved[j=1:n], sum(sum(z_r[ln,rr] for rr in R_allpickups[ln][j]) for ln in 1:numlines) <= 1);
    @constraint(rmp, transferconsistency[j=1:n], sum(sum(z_r[ln,rr] for rr in R_transpickups[ln][j]) for ln in 1:numlines) == sum(sum(z_r[ln,rr] for rr in R_transdropoffs[ln][j]) for ln in 1:numlines));
    @constraint(rmp, dropoffafterpickup[ff=1:f], sum(sum(z_r[ln,rr] for rr in R_one[ln][ff]) for ln in 1:numlines) <= sum(sum(z_r[ln,rr] for rr in R_two[ln][ff]) for ln in 1:numlines));

    # objective
    @objective(rmp, Max, sum(sum(z_r[ln,rr] * rset[ln][rr].g_r for rr in 1:length(rset[ln])) for ln in 1:numlines));
    ...
    localrset = enumerateroutes_reflines()
    ...
    while cg_iter <= 1000
        ...
        sptimestart = time()
        newroute = solvesubproblem_reflines(dualinfo, localrset)   
        sptime += time() - sptimestart

		#---------ADD NEW VARIABLES---------#

		for ln in 1:numlines
    		if newroutes[ln] != nothingroute

                newroute = newroutes[ln]

                #Add to sets
                push!(rset[ln], RouteSet(newroute.start_r,newroute.end_r,newroute.g_r,newroute.preseta_r,newroute.presetb1_r,newroute.presetb2_r,newroute.nodepath_r,newroute.customer_pudo_r))
                rr = length(rset[ln])
                global timespace_route1[ln] += 1
                global timespace_route2[ln] += 1
                #showsingleroute(string("viz/cg/cgiter", cg_iter, "_route",rr, ".png"), rr, 1800, 1800)

                #Add variable
                global z_r[ln,rr] = @variable(rmp, lower_bound = 0)
                set_name(z_r[ln,rr], string("z_r[",ln,",",rr,"]")) 

                #Add to objective
                set_objective_coefficient(rmp, z_r[ln,rr], newroute.g_r) 

                #Add to constraints
                set_normalized_coefficient(rmpconstraints.con_flowbalance1[newroute.start_r], z_r[ln,rr], 1.0)
                set_normalized_coefficient(rmpconstraints.con_flowbalance1[newroute.end_r], z_r[ln,rr], -1.0)
                for j in newroute.preseta_r
                    set_normalized_coefficient(rmpconstraints.con_ordersserved[j], z_r[ln,rr], 1.0)
                end
                for j in 1:n
                ...
                for j in 1:n
                    if is_noempty_intersect(newroute.presetb1_r, fjset[j])
                        set_normalized_coefficient(rmpconstraints.con_transferconsistency[j], z_r[ln,rr], 1.0)
                    end
                end
                for j in 1:n
                ...
                for ff in newroute.presetb1_r
                    set_normalized_coefficient(rmpconstraints.con_dropoffafterpickup[ff], z_r[ln,rr], 1.0)
                end
                for ff in 1:f
                    if is_noempty_intersect(rset[ln][rr].presetb2_r, ffset[ff])
                        set_normalized_coefficient(rmpconstraints.con_dropoffafterpickup[ff], z_r[ln,rr], -1.0)
        ...
        #println("Iteration $cg_iter r.c. = ", newroute.reduced_cost)
        min_rc = minimum([newroutes[ln].reduced_cost for ln in 1:numlines])
		if (min_rc <= 0.0001) 

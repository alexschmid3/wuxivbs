...
function Procedure_DP_for_bus_jj_cg(lset::Vector{LabelSet}, Start_node::Int, Start_time::Int, index_start::Int, e::Int, ll::Int, bus_jj::Int, printstatements::Int, localrset::Vector{TempRouteSet}, dualinfo)
    se = lset[e]
...
"""
function Procedure_DP_for_check_arc_cg(lset::Vector{LabelSet}, Start_time::Int, index_start::Int, ll::Int, allstations::Vector{Int}, alltimes::Vector{Float64}, allorders::Vector{Tuple{Int64,Int64,Base.Int64}}, localrset::Vector{TempRouteSet}, dualinfo)
    se = lset[ll]
...
function Procedure_DP_for_generating_arcs_cg(lset::Vector{LabelSet}, Start_node::Int, Start_time::Int, index_start::Int, ll::Int, printstatements::Int, localrset::Vector{TempRouteSet}, dualinfo)
    ...
    if se.Q_l == 0 && se.S_l > 0
        ...
        localrset = Procedure_DP_for_check_arc_cg(lset, Start_time, index_start, ll, allstations, alltimes, allorders, localrset, dualinfo)
        return localrset
    end                                                                                             
    ...
    for bus_jj in union(bus_ii:TOTAL_Station, bus_ii - 1:-1:1)
        localrset = Procedure_DP_for_bus_jj_cg(lset, Start_node, Start_time, index_start, e, ll, bus_jj, printstatements, localrset, dualinfo)
    end
...
function solvesubproblem_recursion(dualinfo)
    ...
    for s_1 in 2:s
        if n_location_s[s_1] != TOTAL_Station + 1 && n_location_s[s_1] != 0 
            ...
            localrset = Procedure_DP_for_generating_arcs_cg(lset, n_location_s[s_1], time_s[s_1] * unit, s_1, 1, 0, localrset, dualinfo)
            ttm = time()-stm
            #if ttm > 0.1
            #    println("s_1 = $s_1 --> ", ttm)
            #end

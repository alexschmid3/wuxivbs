...
"""
function Create_time_space_nodes_and_arcs(W::Int)
    ...
    # ----------------
    # ii. Create all time-space arcs r
    # ----------------
    ...
    println("Initial routes = ", length(rset))
    enumstarttime = time()
    for s_1 in 2:s
        if n_location_s[s_1] != TOTAL_Station + 1 && n_location_s[s_1] != 0 
            ...
            stm=time()
            Procedure_DP_for_generating_arcs(lset, n_location_s[s_1], time_s[s_1] * unit, s_1, 1, 0)
            ttm = time()-stm
            #if ttm > 0.1
            #    println("s_1 = $s_1 --> ", ttm)
            #end
    ...
    timespace_route1 = length(rset)
    println("Movement subpaths = $timespace_route1")

    for s_1 in 1:s # Start_and_End
        s1_loc, s1_time = n_location_s[s_1], time_s[s_1]

        #Add arcs from dummy origin to start locations
        if (s1_loc == 0)
            for s2_loc in 1:TOTAL_Station
                try
        ...
        #Add arcs to dummy destination from end locations
    ...
    r = length(rset)
    timespace_route2 = length(rset) - timespace_route1
    println("Dummy and holding arcs = $timespace_route2")
    println("Total arcs = $r")
